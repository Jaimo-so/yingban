...
class VoiceConfig:
    ...
    def enabled(self) -> bool:
        credentials_ready = bool(self.api_key) if self.provider == "openai_compatible" else bool(
...
class VoiceRuntime:
    """OpenAI-compatible and Doubao speech-to-text/text-to-speech adapters."""
    ...
    def config(self, overrides: dict[str, Any] | None = None) -> VoiceConfig:
        ...
        if overrides:
            ...
            for key in ("provider", "app_id", "base_url", "stt_model", "tts_model", "voice_name", "audio_format", "timeout_seconds"):
                if key in overrides:
                    current[key] = overrides[key]
        provider = str(current["provider"]).strip()
        if provider not in {"openai_compatible", "doubao"}:
            raise ValueError("语音服务商只支持 OpenAI 兼容或豆包")
        ...
        values = {
            ...
            for name in ("stt_model", "tts_model", "voice_name", "audio_format")
        }
        ...
        return VoiceConfig(provider, api_key, app_id, access_token, _url(current["base_url"]), values["stt_model"], values["tts_model"], values["voice_name"], values["audio_format"], timeout)
